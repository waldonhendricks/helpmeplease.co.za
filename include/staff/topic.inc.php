...
<table width="100%" border="0" cellspacing=0 cellpadding=2 class="tform">
    ...
    <tr class="subheader">
        <td colspan=2 >Disabling auto response will overwrite domain settings.</td>
    </tr>
    <tr>
        <th width="20%">Query Type:</th>
        <td><input type="text" name="topic" size=45 value="<?=$info['topic']?>">
            &nbsp;<font class="error">*&nbsp;<?=$errors['topic']?></font></td>
    </tr>
    <tr><th>Query Status</th>
        <td>
    ...
    <tr>
        ...
        <td>
            <input type="checkbox" name="noautoresp" value=1 <?=$info['noautoresp']? 'checked': ''?> >
                <b>Disable</b> autoresponse for this query.   (<i>Overwrite Domain setting</i>)
        </td>
    </tr>
    <tr>
        ...
        <td>
            <select name="dept_id">
                ...
                while (list($id,$name) = db_fetch_row($depts)){
                    ...
                    <option value="<?=$id?>"<?=$selected?>><?=$name?> Domain</option>
                <?
                }?>
