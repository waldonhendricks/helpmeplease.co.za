...
<html>
<head>
<title>HHI Support Report Top Ten Ticket opening sites of <? echo $year ?></title>
...
$query = "select `agency`,COUNT(`ticket_id`) as `counted` FROM `ost_ticket` WHERE YEAR(created)='$year' GROUP by `agency` ORDER BY `counted` DESC LIMIT 0,10;";
...
<b>Top Ten Ticket Opening sites of <? echo $year ?></b><br>
...
<? 
 ...
 while ($i < $num) {
  ...
  $getsite = "select agency from ost_agencies where id='$agency'";	
	...
	print "<b>".$actnum."</b> &nbsp; $site ($counted) <br>";
...
<br>
<a href="javascript:toggleLayer('EmpForm');" title="See Data: Locations">See Raw Data: Top Ten Ticket opening sites of <? echo $year ?></a><br>
<div id="EmpForm">
